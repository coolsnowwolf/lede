...
static bool qcom_scm_qseecom_machine_allowed(void)
{
	int i;

	for (i = 0; qcom_scm_qseecom_allowlist[i].compatible; i++)
		if (of_machine_is_compatible(qcom_scm_qseecom_allowlist[i].compatible))
			return true;
...
static int qcom_scm_qseecom_init(struct qcom_scm *scm)
{
	...
	if (!qcom_scm_qseecom_machine_allowed()) {
