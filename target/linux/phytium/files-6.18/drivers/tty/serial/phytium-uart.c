...
{
	typeof(pup->port.state->port.xmit_fifo) *xmit =
		&pup->port.state->port.xmit_fifo;
	u8 ch;
	...
	if (kfifo_is_empty(xmit) || uart_tx_stopped(&pup->port)) {
	...
	do {
		...
		if (!kfifo_peek(xmit, &ch) ||
		    !phytium_tx_char(pup, ch, from_irq))
			break;
			      ...
			      const struct pci_device_id *id)
{
	...
	pup->port.membase = pcim_iomap_region(pdev, 0, pci_name(pdev));
	if (IS_ERR(pup->port.membase))
		return PTR_ERR(pup->port.membase);
