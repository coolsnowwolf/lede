...
static void pwm_phytium_set_periodns(struct pwm_chip *chip, unsigned int periodns, int n)
{
	...
	if (has_acpi_companion(pwmchip_parent(chip)))
		device_property_read_u64(pwmchip_parent(chip), "clock-frequency", &cycles);
	else
...
{
	return __pwm_phytium_config(chip, pwm);
}
				       ...
				       struct device_node *np)
{
	...
	if (has_acpi_companion(pwmchip_parent(priv->chip))) {
		priv->num_parameters = 1;
		array_size = sizeof(struct phytium_pwm_param) / sizeof(u32);
		ret = fwnode_property_read_u32_array(dev_fwnode(pwmchip_parent(priv->chip)),
...
static int pwm_phytium_probe(struct platform_device *pdev)
{
	...
	pwmchip = devm_pwmchip_alloc(dev, PWM_NUM, sizeof(*chip));
	if (IS_ERR(pwmchip))
...
static void pwm_phytium_remove(struct platform_device *pdev)
{
