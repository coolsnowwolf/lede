...
static void phytium_spi_set_cs(struct spi_device *spi, bool enable)
{
	...
	if (!enable) {
		phytium_writel(fts, SER, BIT(spi_get_chipselect(spi, 0)));
		if (fts->global_cs) {
			origin = phytium_readl(fts, GCSR);
			phytium_writel(fts, GCSR, origin | (1 << spi_get_chipselect(spi, 0)));
		}
	} else {
		if (fts->global_cs) {
			origin = phytium_readl(fts, GCSR);
...
static int phytium_spi_transfer_one(struct spi_controller *master,
		struct spi_device *spi, struct spi_transfer *transfer)
{
	...
	if (master->can_dma && master->can_dma(master, spi, transfer))
		fts->dma_mapped = spi_xfer_is_dma_mapped(master, spi, transfer);
...
static void phytium_spi_handle_err(struct spi_controller *master,
