...
int gpon_recover_delete_allocId(unsigned short allocId)
{
	...
	for(i = 0; i < gponRecovery.gemPortNum; )
		{
		if((allocId != bakGemport[i].allocId) || (0 != gwan_remove_gemport(bakGemport[i].gemPortId)))
			{
			++i;
		}
	}
...
Ret Value:
	...
	if fali, return non-zero error code
*********************************************************************************************************************/
int gpon_recovery_set_encrytion(unsigned short gemPortId, unsigned int encrytion)
{
    struct Gem_Recovery_Info * tmp = NULL;
    
    tmp = find_backup_gemport(gemPortId);

    DBG_INFO(gponRecovery.dbgPrint, "gemPort = %d, encrytion = %d\n",gemPortId,encrytion);

    if(NULL != tmp){
        tmp->encryption = (unsigned char)encrytion;
    }else{
...
int gpon_recover_get_real_channel(ushort allocId)
{
	...
	for(channel = 0; channel < GPON_TCONT_MAX_NUM; channel++)
	{
...
int gpon_check_gem_encrypt(void)
{
	int i = 0;
	for(i=0 ; i<GPON_GEMPORT_MAX_NUM ; i++) 
	{
		if(gpWanPriv->gpon.gemPort[i].info.rxEncrypt){
			return 1;
		}
	}
    return 0;
...
Ret Value:
	None
*********************************************************************************************************************/
void gpon_recovery_set_channel(unsigned short gemPortId, unsigned int channel)
{
    struct Gem_Recovery_Info * tmp = NULL;
...
int gpon_recover_create_gemport(void)
{
	...
	DBG_INFO(gponRecovery.dbgPrint, "gemport num:%d\n",gponRecovery.gemPortNum);
	...
	for(i = 0; i < gponRecovery.gemPortNum; i++)
	{
		...
		if(gpon_check_gem_exist(pBakGemPort->gemPortId) == 1){			
			DBG_INFO(gponRecovery.dbgPrint, " gem %d exist\n", gponRecovery.gemPort[i].gemPortId);
			continue;
		}

		channel = gpon_recover_get_real_channel(pBakGemPort->allocId);
		if((-1 == channel) && GPON_MULTICAST_CHANNEL != pBakGemPort->channel){
		...
		if(0 != gwan_create_new_gemport(pBakGemPort->gemPortId, pBakGemPort->channel, pBakGemPort->encryption,pBakGemPort->allocId)){
