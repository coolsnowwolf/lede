...
extern void delay1us(int us);
extern uint dump_all_gemport;
extern REG_G_INT_STATUS intStatusDbg ;
...
void select_xpon_wan(Xpon_Phy_Mode_t mode)
{
	...
	switch(mode)
	{
		...
		case PHY_GPON_SYM_CONFIG:
		 /* Set the SCU register to select the WAN mode (GPON_SYM) */
		reg &= ~(0xff);
		reg |= (0x14 & 0xff); 
		break;
...
int gponDevGetPloamMsg(PLOAM_RAW_General_T *pPloamMsg)
{
	...
	if(gponPloamStatus.Bits.ploamd_fifo_used == 0) {
		...
		for(i=0 ; i<3 ; i++) {
			pPloamMsg->value[i] = ntohl(IO_GREG(G_PLOAMd_RDATA)) ;
		}
	} else {
		return -EFAULT ;
	}
...
int gponDevSendPloamMsg(PLOAM_RAW_General_T *pPloamMsg, uint times)
{
	...
	for(i=0 ; i<times ; i++) {
		for(j=0 ; j<3 ; j++) {
			IO_SREG(G_PLOAMu_WDATA, htonl(pPloamMsg->value[j])) ;
		}
...
#endif
{
	...
	/*float  n1310 = 1.4677;
	float  n1490 = 1.4682;	
	float up_rate = 1.24416;	 
	float down_rate = 2.48832; */
	uint64_t  n1310_x10000 = 14677;
	uint64_t  n1490_x10000 = 14682;
	uint64_t up_rate_x100000 = 124416;
	uint64_t down_rate_x100000 = 248832;
	...
	REG_G_RSP_TIME gponRspTime = {0};
	REG_PHY_RX_DLY todRxPhyDly = {0};
	REG_G_TOD_CFG gponTodCfg = {0};
	...
	todRxPhyDly.Raw = IO_GREG(PHY_RX_DLY);
	...
	/*delta =(uint)((((gpGponPriv->gponCfg.eqd + gponRspTime.Bits.tresp * 32)/up_rate) * (n1490/(n1310+n1490))) - (todRxPhyDly.Bits.phy_rx_dly/down_rate) + xpon_tod_adjust);*/

	/*All numbers multiple the same times to avoid float number and maintain the result unchanged.*/
	dividend1 = ((uint64_t)gpGponPriv->gponCfg.eqd + (uint64_t)gponRspTime.Bits.tresp * 32) * 100000;
	divisor1 = up_rate_x100000;
	
	dividend2 = n1490_x10000;
	divisor2 = n1310_x10000 + n1490_x10000;

	dividend3 = (uint64_t)todRxPhyDly.Bits.phy_rx_dly * 100000;
	...
	gponTodCfg.Bits.tod_spf_cnt = new_tod.superframe;
	IO_SREG(G_TOD_CFG, gponTodCfg.Raw) ;

	PON_MSG(MSG_DBG,"gponDevSetNewTod gponTodCfg.Raw = 0x%x, gponTodCfg.Bits.tod_spf_cnt = 0x%x\n",gponTodCfg.Raw, gponTodCfg.Bits.tod_spf_cnt);
	...
	if(isFPGA)
{
		tod_data.tod_add_unit_int = 25;
		tod_data.tod_add_unit_nume = 175;
	...
	uint64_t n1310_x10000 = 14677;
	uint64_t n1490_x10000 = 14682;
	uint64_t onuResponseTime = 35000;
	uint64_t up_x100000 = 124416;
	uint64_t dividend1 = 0;
	uint64_t divisor1 = 0;
	uint64_t dividend2 = 0;
	uint64_t divisor2 = 0;
	...
	/*delta =(uint) (((gpGponPriv->gponCfg.eqd /up) + onuResponseTime1) * n1490/(n1310+n1490));*/

	dividend1 = (uint64_t)gpGponPriv->gponCfg.eqd * 100000;
	divisor1 = up_x100000;
	
	dividend2 = n1490_x10000;
	divisor2 = n1310_x10000 + n1490_x10000;

	/*((dividend1 / divisor1 + onuResponseTime) * dividend2 / divisor2) integer part
	* (dividend1 % divisor1 * dividend2 / (divisor1 * divisor2)) fractional part	
	*/
	delta = (uint)(((dividend1 / divisor1 + onuResponseTime) * dividend2 / divisor2) + (dividend1 % divisor1 * dividend2 / (divisor1 * divisor2)));
	if (delta > nanosec)
	{
		sec = sec -1;
...
int gponDevDumpGemInfo(void)
{
	...
	for(i=0 ; i < GPON_MAX_GEM_ID ; i++) {
		...
		if(ret != 0) {
			...
			if((dump_all_gemport) || (XPON_ENABLE == valid))
			{
...
int gponDevUpdateGemMibIdxTable(unchar addr, ushort data)
{
	...
	if (isEN7580 || isEN7528) {
	/*SW resolve HW issue: read/write GPIDX value is equal to the last read/write address value,
		but before read/write GPIDX value Read Command second can resolve this problem*/
		gponGPIdxTblCtrl.Raw = 0 ;
		gponGPIdxTblCtrl.Bits.gpidx_cmd = 0 ; //read Command
...
/*****************************************************************************
******************************************************************************/
int gponDevGetGemMibIdxTable(unchar addr, uint *data)
{
	int RETRY = 3 ;
	REG_G_GPIDX_TBL_CTRL gponGPIdxTblCtrl ;
	REG_G_GPIDX_TBL_STS gponGPIdxTblStatus ;
	...
	/*SW resolve HW issue: read/write GPIDX value is equal to the last read/write address value,
		...
		RETRY = 3 ;
		gponGPIdxTblCtrl.Raw = 0 ;
		gponGPIdxTblCtrl.Bits.gpidx_cmd = 0 ; //Read Command second 
		gponGPIdxTblCtrl.Bits.gpidx_tbl_addr = addr ;
		IO_SREG(G_GPIDX_TBL_CTRL, gponGPIdxTblCtrl.Raw) ;
	}	
	while(RETRY--) {
                    ...
                    uint64_t *pData, unchar readClear)
{
	...
	while(RETRY--) {
		...
		if(gponGemMibCtrl.Bits.mib_cmd_done) {
			tmpData = IO_GREG(G_MIB_RDATA_H32);
			tmpData = tmpData << 32;
			tmpData = tmpData + IO_GREG(G_MIB_RDATA_L32);
			*pData = tmpData;
			return 0 ;
		} 
		mdelay(1) ;
...
/*****************************************************************************
******************************************************************************/
int gponDevGetGemPortCounter(ushort gemPortId, GPON_GEMPORT_STATS_TYPE_t type, uint64_t *pData, unchar readClear)
{
    return __gponDevGetGemPortCounter(gemPortId, type, pData, readClear);
}
...
int gponDevSetSniffMode(GPON_DEV_SNIFFER_MODE_T *sniffer)
{
	...
	REG_DBG_SNIFF_DBG_REG           dbg_sniff;
	int gtc_reset_val = 1;
#endif
	...
	if(isFPGA)
	{
	   	if(sniffer->ds_filter_enable == XPON_DISABLE){
			IO_SREG(SNIFF_GTC_GTC_INVLD_GEM_BYTE, 0x1de2) ; //packet byte 4k
		}else{
			IO_SREG(SNIFF_GTC_GTC_INVLD_GEM_BYTE, 0x8001de2) ; 
		}
	}else{
#if defined(TCSUPPORT_CPU_EN7581) || defined(TCSUPPORT_CPU_AN7552)
		IO_SREG(SNIFF_GTC_GTC_INVLD_GEM_BYTE, 0x1de2) ;
#endif
		if(sniffer->ds_filter_enable == XPON_DISABLE){
			IO_CBITS(SNIFF_GTC_GTC_INVLD_GEM_BYTE, SNIFF_FILTER_DS_GEM_STATUS);
		}else{
			IO_SBITS(SNIFF_GTC_GTC_INVLD_GEM_BYTE, SNIFF_FILTER_DS_GEM_STATUS);
		}
	}
...
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7528)
    /*reset gtc*/
    while(gtc_reset_val >= 0)
    {
        dbg_gtc_eth_extr.Raw = IO_GREG(DBG_GTC_ETH_EXTR) ;

        dbg_gtc_eth_extr.Bits.sniff_dbg_tx_rst = gtc_reset_val;
        IO_SREG(DBG_GTC_ETH_EXTR, dbg_gtc_eth_extr.Raw) ;

        gtc_reset_val--;
    }
    dbg_sniff.Raw = IO_GREG(DBG_SNIFF_DBG_REG) ;
    dbg_sniff.Bits.dbg_us_gtc_info_fifo_ovrn = 1;
    dbg_sniff.Bits.dbg_us_gtc_sfc_fifo_ovrn = 1;
    dbg_sniff.Bits.dbg_us_gtc_sfc_fifo_udrn = 1;
    IO_SREG(DBG_SNIFF_DBG_REG, dbg_sniff.Raw) ;
#endif
	
#if defined(TCSUPPORT_CPU_EN7581) || defined(TCSUPPORT_CPU_AN7552)
	if(XPON_DISABLE == sniffer->ds_enable)
	{
		times = 0;
		count = 0;

		dbg_gtc_eth_extr.Raw = IO_GREG(DBG_GTC_ETH_EXTR);
		dbg_gtc_eth_extr.Bits.dbg_ds_snf_en = sniffer->ds_enable;

	IO_SREG(DBG_GTC_ETH_EXTR, dbg_gtc_eth_extr.Raw) ;

		while((times < 4) && (count < 3000))
	{
			udelay(1);
		dbg_sniff.Raw = IO_GREG(DBG_SNIFF_DBG_REG) ;
		
		if(XPON_ENABLE == dbg_sniff.Bits.dbg_ds_snf_fifo_empty)
		{
				times++;			
			}
			else
		...
		if(4 == times)
		{
			printk("Gpon disable ds sniffer success!\n");
		}
		else
		{
			printk("Gpon disable ds sniffer fail!\n");
		}
	
	}	

	if(XPON_DISABLE == sniffer->us_enable)
	{
		times = 0;
		count = 0;

		dbg_gtc_eth_extr.Raw = IO_GREG(DBG_GTC_ETH_EXTR);
		dbg_gtc_eth_extr.Bits.dbg_us_snf_en = sniffer->us_enable;
		...
		while((times < 4) && (count < 3000))
		{
			...
			if(XPON_ENABLE == dbg_sniff.Bits.dbg_us_snf_fifo_empty)
			{
				times++;			
			}
			else
		...
		if(4 == times)
		{
			...
			dbg_gtc_eth_extr.Bits.dbg_us_snf_window_en = 0;
			IO_SREG(DBG_GTC_ETH_EXTR, dbg_gtc_eth_extr.Raw);
	
			dbg_gtc_eth_extr.Raw = IO_GREG(DBG_GTC_ETH_EXTR);
			if(XPON_DISABLE == dbg_gtc_eth_extr.Bits.dbg_us_snf_window_en)
			{				

			printk("Gpon disable us sniffer success!\n");

			}
			else
			{
				printk("Gpon disable us sniffer fail, dbg_us_snf_window_en = %d !\n",dbg_gtc_eth_extr.Bits.dbg_us_snf_window_en);
			}
		}
		else
		{
			printk("Gpon disable us sniffer fail, times = %d, count = %d\n",times,count);
		}
		}
#endif
	...
	if(XPON_ENABLE == sniffer->us_enable)
	{
		dbg_gtc_eth_extr.Bits.dbg_us_snf_window_en = 1;
	}
#endif
...
int gponDevSwResync(void)
{
	...
	IO_SREG(DBG_SW_RESYNC, swResync.Raw) ;
	delay1us(GPON_MAC_SW_RESYNC_INTERVAL_US) ;
...
/******************************************************************************
 ...
 Input Args:	tcont_index: tcont index from 0 to (GPON_TCONT_MAX_NUM - 1).
...
GPON_TCONT_CMD_RESULT_t gponDevSetTCont(GPON_TCONT_t isValid, int tcont_index, ushort allocId)
{
	...
	if(tcont_index < 0 || tcont_index >= GPON_TCONT_MAX_NUM) {
		return GPON_TCONT_CMD_FAIL;
	}
...
int gponDevDisableAllTCont(void)
{
	int i;
	GPON_TCONT_CMD_RESULT_t result;
	for(i = 0 ; i < GPON_TCONT_MAX_NUM ; i++){
...
int gponDevEnableTCont(ushort allocId)
{
	 ...
	 */
	for(i = 1 ; i < GPON_TCONT_MAX_NUM ; i++) {
		/* read tcont status */
		result = gponDevGetTCont(&valid, i, &id);
		if(result == GPON_TCONT_CMD_SUCCESS) {
...
void gponDevSetShadowKey(GPON_DEV_ENCRYPT_KEY_INFO_T *aesKey)
{
	IO_SREG(G_AES_SHADOW_KEY0, aesKey->activeKey[0]) ;
	IO_SREG(G_AES_SHADOW_KEY1, aesKey->activeKey[1]) ;
	IO_SREG(G_AES_SHADOW_KEY2, aesKey->activeKey[2]) ;
	IO_SREG(G_AES_SHADOW_KEY3, aesKey->activeKey[3]) ;
}
...
******************************************************************************/
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7528)
int gponDevSetDyingGaspIntvl(uint intvl)
{
	REG_DBG_US_DYING_GASP_CTRL gponDGCtrl ;
	
	gponDGCtrl.Raw = IO_GREG(DBG_US_DYING_GASP_CTRL) ;
	gponDGCtrl.Bits.dying_gasp_intvl = intvl ;
	IO_SREG(DBG_US_DYING_GASP_CTRL, gponDGCtrl.Raw) ;
...
/*****************************************************************************
******************************************************************************/
int gponDevGetDyingGaspIntvl(uint *intvl)
{
	REG_DBG_US_DYING_GASP_CTRL gponDGCtrl ;
	
	gponDGCtrl.Raw = IO_GREG(DBG_US_DYING_GASP_CTRL) ;
	*intvl = gponDGCtrl.Bits.dying_gasp_intvl ;
...
int gponDevClearHwCounter(void)
{
	...
	for(j = 1, i = 0 ; i < GPON_MAX_GEM_ID ; i++) {
		...
		PON_MSG(MSG_DBG, "GPON dev clear hw counter i = %d, j = %d, gemIdx = %x, idx = %d\n",i,j,gemIdx,idx);
		if((i&0xF)==0xF && j!=256) {
...
#if defined(TCSUPPORT_CPU_EN7528) || defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7523)
/*****************************************************************************
******************************************************************************/
int gponDevSetRdiSendNum(ushort num)
{
	REG_DBG_SW_RDI_CTRL rdiCtrl;

	rdiCtrl.Raw = IO_GREG(DBG_SW_RDI_CTRL);
	rdiCtrl.Bits.rdi_num = num;
	IO_SREG(DBG_SW_RDI_CTRL, rdiCtrl.Raw);
...
/*****************************************************************************
******************************************************************************/
int gponDevSetRdiErrCtrlSwitch(unchar switchBitMask)
{
	REG_DBG_RDI_ERR_CTRL rdiErrCtrl;

	rdiErrCtrl.Raw = IO_GREG(DBG_RDI_ERR_CTRL);
	rdiErrCtrl.Bits.gem_del_err_rid_en = (switchBitMask & 0x80)?1:0;
	rdiErrCtrl.Bits.ploamd_crc_err_rid_en = (switchBitMask & 0x40)?1:0;
	rdiErrCtrl.Bits.bwm_uc_err_rdi_en = (switchBitMask & 0x20)?1:0;
	rdiErrCtrl.Bits.blen_err_rdi_en = (switchBitMask & 0x10)?1:0;
	rdiErrCtrl.Bits.eth_fcs_err_rdi_en = (switchBitMask & 0x08)?1:0;
	IO_SREG(DBG_RDI_ERR_CTRL, rdiErrCtrl.Raw);
	
	return 0 ;
}

int gponDevSetRdiErrCtrlThreshold(uint threshold)
{
	REG_DBG_RDI_ERR_CTRL rdiErrCtrl;

	rdiErrCtrl.Raw = IO_GREG(DBG_RDI_ERR_CTRL);
	rdiErrCtrl.Bits.rdi_send_err_thld = threshold & 0x7FFFFFF;
	IO_SREG(DBG_RDI_ERR_CTRL, rdiErrCtrl.Raw);
...
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7528)
int gponDevMpiStop(XPON_Mode_t mode)
{
	REG_G_MBI_MPI_STOP gponMpiStop ;
	int RETRY = 200 ;
	...
	/* STOP/START MBI Interface */
	gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	/* 0: not stop 
	 *  1: stop
	 */
	gponMpiStop.Bits.mpi_rx_stop = (mode == XPON_DISABLE) ? 0 : 1 ;
	gponMpiStop.Bits.mpi_tx_stop = (mode == XPON_DISABLE) ? 0 : 1 ;
	IO_SREG(G_MBI_MPI_STOP, gponMpiStop.Raw) ;

	/* wait for MPI interface finish */
    if(mode == XPON_ENABLE){
    	while(RETRY--) {
    		gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
    		if((gponMpiStop.Bits.mpi_rx_stop_done)&&(gponMpiStop.Bits.mpi_tx_stop_done)) {
    			break  ;
    		}
			delay1us(1);
    	}
...
#endif
int gponDevResetCtrl(XPON_Mode_t mode)
{
    unsigned long flags;
    REG_G_ONU_ID gponOnuId ;
...
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7528)
        gponDevMpiStop(XPON_ENABLE) ;
#else
        delay1us(1);
#endif
        if(!gpGponPriv->gponCfg.sw_reset_flag)
        {
#ifdef TCSUPPORT_CPU_ARMV8
			/* reset mac */
			raw = GET_SCU_RSTCTRL1();
			raw |= SCU_GPON_MAC_RESET  ;
...
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7528)
        gponDevMpiStop(XPON_DISABLE) ;
#else
        delay1us(1);
#endif 
        IO_SREG(DBG_GRP_0, 0xffffffff) ;
        IO_SREG(DBG_GRP_1, 0xffffffff) ;
		memset(&intStatusDbg, 0x0, sizeof(REG_G_INT_STATUS));	 
        //IO_SREG(0xbfb643cc, 0x0) ;
        //IO_SREG(DBG_PROBE_CTRL, 0x000b0006) ;
    }
...
static void gpon_INT_init(void)
{
	...
	gponIntEnable.Raw = 0 ;
#ifdef TCSUPPORT_CPU_EN7521
	gponIntEnable.Bits.bwm_us_fec_err_int_en     = 1;
...
#else
    gponIntEnable.Bits.cal_gnt_size_done_int_en =   1 ;
#endif
#endif
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7528)
    gponIntEnable.Bits.sn_req_recv_o5_int_en =      0 ;
#endif
...
int gpon_dev_init(void)
{
    ...
    if(gpGponPriv->gponCfg.consid_tx_rx_ph)
    {
        IO_SREG(CONSID_TX_RX_PH, 0) ;
    }

#if defined(TCSUPPORT_CPU_EN7528) || defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_CPU_EN7523)
	gponDevSetRdiSendNum(gpGponPriv->gponCfg.rdiSendNum);
#if defined(TCSUPPORT_CPU_EN7523) || defined(TCSUPPORT_CPU_EN7581)
	gponDevSetRdiErrCtrlSwitch(gpGponPriv->gponCfg.rdiErrCtrlEn);
	gponDevSetRdiErrCtrlThreshold(gpGponPriv->gponCfg.rdiErrCtrlThreshold);
#endif
#endif

#if defined(TCSUPPORT_CPU_AN7552) || defined(TCSUPPORT_CPU_AN7583)
		/* just for GPON SYM Mode */
		if(gpPhyData->working_mode == PHY_GPON_SYM_CONFIG) {
			
			REG_DBG_CAP_SETTING gponCapSet ;
			REG_DBG_US_PREF_OFS gponUsPrefOfs ;
			REG_DBG_DLY dbgDly;
	
			gponCapSet.Raw = IO_GREG(DBG_CAP_SETTING);
			if(!gpGponPriv->gponCfg.flags.maxRdmDlyFlag){
				gponCapSet.Bits.max_rdm_dly = 0x1d2;
			}
	
			gponUsPrefOfs.Raw = IO_GREG(DBG_US_PREF_OFS);
			gponUsPrefOfs.Bits.dt_pref_ofs = 0x800;
			
			dbgDly.Raw = IO_GREG(DBG_DLY);
			dbgDly.Bits.phy_tx_dly = 0x40;
	
			IO_SREG(DBG_CAP_SETTING, gponCapSet.Raw) ;
			IO_SREG(DBG_US_PREF_OFS, gponUsPrefOfs.Raw);
			IO_SREG(DBG_DLY, dbgDly.Raw);
	
			gpGponPriv->gponCfg.onuResponseTime = 0xaee; 
			gponDevSetResponseTime(gpGponPriv->gponCfg.onuResponseTime);
		} 
#endif
	return 0 ;
...
void gpon_dev_typeB_restore(void)
{
    REG_G_ONU_ID gponOnuId ;
    REG_G_GBL_CFG gponGlbCfg ;
    REG_G_OMCI_ID gponOmciId ;
...
int gponDevCheckTContReg(ushort channelId, ushort allocId)
{
	...
	uint TcontReg = 0;
	GPON_TCONT_t isValid = 0;
	ushort allocId_tmp = 0;

	if(channelId >= GPON_TCONT_MAX_NUM || allocId >= GPON_MAX_ALLOC_ID)
	{
	...
	if(channelId >= 16) {
#ifdef TCSUPPORT_CPU_EN7521
		if(0 == gponDevGetTCont16_31(&isValid, channelId, &allocId_tmp)){
			if((1 == isValid) && (allocId == allocId_tmp))
			{
			...
			else
			{
				printk("ERROR: Tcont info in reg is wrong,channelId %d, valid %d, alloc %d\n",
					channelId,isValid,allocId_tmp);
				return FALSE;
			}

		}
		printk("ERROR: gponDevGetTCont16_31 Fail, channelId: %d	allocId: %d\n",channelId,allocId);
		return FALSE;
#else
		printk("ERROR: Input Para wrong, channelId: %d	allocId: %d\n",channelId,allocId);
		return FALSE;
	...
	TcontReg = (uint)G_TCONT_ID(channelId / 2);
...
void gponDevGetErrStatus(GPON_DEV_ERR_STATUS_T *status)
{
	...
	dbg_grp_0.Raw =	IO_GREG(DBG_GRP_0) ;
	status->dbg_grp_0.Raw = dbg_grp_0.Raw;

	dbg_grp_1.Raw =	IO_GREG(DBG_GRP_1) ;
	status->dbg_grp_1.Raw = dbg_grp_1.Raw;
	
	intStatus.Raw =	intStatusDbg.Raw ;
	status->intStatus.Raw = intStatus.Raw;
}
/******************************************************************************
...
#if defined(TCSUPPORT_CPU_EN7581) || defined(TCSUPPORT_CPU_EN7523)
void gpon_typeb_dev_reset_for_typeB(void)
{
	...
	/* STOP DEL GEM RX Interface */
	gponDelStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponDelStop.Bits.del_rx_stop = XPON_ENABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponDelStop.Raw) ;
	
	/* STOP MPI Rx Interface */
	gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMpiStop.Bits.mpi_rx_stop = XPON_ENABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMpiStop.Raw) ;
	/* wait for MPI interface finish */
	while(RETRY--) {
		gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
		if(gponMpiStop.Bits.mpi_rx_stop_done) {
			break  ;
		}
	}	

	/* STOP MBI Rx Interface */
	gponMbiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMbiStop.Bits.mbi_rx_stop = XPON_ENABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMbiStop.Raw) ;
	/* wait for MBI interface finish */
	RETRY = 3000 ;
	while(RETRY--) {
		gponMbiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
		if(gponMbiStop.Bits.mbi_rx_stop_done) {
			break  ;
		}
	}

	/* STOP MBI Tx Interface */
	gponMbiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMbiStop.Bits.mbi_tx_stop = XPON_ENABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMbiStop.Raw) ;
	/* wait for MBI interface finish */
	RETRY = 3000 ;
	while(RETRY--) {
		gponMbiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
		if(gponMbiStop.Bits.mbi_tx_stop_done) {
			break  ;
		}
	}	

	/* STOP MPI Tx Interface */
	gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMpiStop.Bits.mpi_tx_stop = XPON_ENABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMpiStop.Raw) ;
	/* wait for MPI interface finish */
	RETRY = 3000 ;
	while(RETRY--) {
		gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
		if(gponMpiStop.Bits.mpi_tx_stop_done) {
			break  ;
		}
	}
	...
	/* START MPI Tx Interface */
	gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMpiStop.Bits.mpi_tx_stop = XPON_DISABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMpiStop.Raw) ;

	/* START MBI Tx Interface */
	gponMbiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMbiStop.Bits.mbi_tx_stop = XPON_DISABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMbiStop.Raw) ;

	/* START MBI Rx Interface */
	gponMbiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMbiStop.Bits.mbi_rx_stop = XPON_DISABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMbiStop.Raw) ;
	
	/* START MPI Rx Interface */
	gponMpiStop.Raw = IO_GREG(G_MBI_MPI_STOP) ;
	gponMpiStop.Bits.mpi_rx_stop = XPON_DISABLE ;
	IO_SREG(G_MBI_MPI_STOP, gponMpiStop.Raw) ;
}
#endif
