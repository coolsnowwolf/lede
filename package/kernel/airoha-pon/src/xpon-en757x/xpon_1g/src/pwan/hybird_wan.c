...
static int is_route_mode(char * mac)
{
    ...
    spin_lock_bh(&pwan_hybridData.macLock);
...
IS_ROUTE_MAC_RETURN:
    spin_unlock_bh(&pwan_hybridData.macLock);
    return ret;
}
	...
	unsigned long count, void *data)
{
    ...
    unsigned char mac[ETH_ADDR_LEN][HYBRID_PROC_MAX_STRING];
	...
	val_string[count] = '\0';
    if (13 == sscanf(val_string, "route mac%s %c%c%c%c:%c%c%c%c:%c%c%c%c",(char *)(&idx), \
					&mac[0][0], &mac[0][1],
					&mac[1][0], &mac[1][1],
					&mac[2][0], &mac[2][1],
					&mac[3][0], &mac[3][1],
					...
					&mac[5][0], &mac[5][1]))
    {
	    ...
	    return count;	
    }else if (12 == sscanf(val_string, "my mac %s %c%c%c%c:%c%c%c%c:%c%c%c%c", (char *)(&idx),\
					&mac[0][0], &mac[0][1],
					&mac[1][0], &mac[1][1],
					&mac[2][0], &mac[2][1],
					&mac[3][0], &mac[3][1],
					...
					&mac[5][0], &mac[5][1]))
    {
        ...
        printk("Set my mac valid is %d to %02x%02x:%02x%02x:%02x%02x\n",pwan_hybridData.myMac.valid,
            ...
            pwan_hybridData.myMac.macAddr[4],pwan_hybridData.myMac.macAddr[5]);
	    return count;	
    }else if (1 == sscanf(val_string, "disable route mac%s",(char *)(&idx))) {
        val = simple_strtoul(idx,NULL,0);
        if( val <0 || val >= HYBIRD_ROUTE_MAC_MAX )
            ROUTE_MAC_USAGE_PRINT_WITH_RET(count); 
...
int hybird_pwan_net_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    PWAN_NetPriv_T *pNetPriv_pon = NULL;
        ...
        0 == memcmp(dev->name,"wan0",sizeof("wan0")))
    {
        ...
        PON_MSG(MSG_TRACE,"[%s][%d] :new device name = %s.\n",__FUNCTION__,__LINE__,dev->name);
    }
