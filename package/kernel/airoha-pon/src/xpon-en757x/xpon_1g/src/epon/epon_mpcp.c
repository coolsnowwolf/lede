...
int epon_random_delay_test = 0;
int random_result[2000] = {0};
int random_delay_test_cnt = 0;
int random_dly_cnt = 0;
...
int isAuthFail = 0;
int isSlientBegin = 0;
__u8 RgstrAckFlag[8] = {ACK,ACK,ACK,ACK,ACK,ACK,ACK,ACK};

extern void setMpcpFwd(int flag);

__u8 g_silence_time = MPCP_RGST_AGAIN_TIMEOUT;
extern __u32 g_epon_llid_dscv_stat[EPON_1G_MAX_LLID_NUM];
extern struct tasklet_struct eponStartTask;
...
ssize_t max_dscv_gate_read_proc(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int len;
	loff_t off = *ppos;
...
//
******************************************************************************/
ssize_t max_dscv_gate_write_proc(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	char val_string[8];
	int cnt;
...
void eponSetGrantLength(void)
{
    unsigned int tmp = 0;
    static unsigned int lenCnt = 0;
    eponGrtLen_t  * value = &(gpEponPriv->eponRdmMfy);

    /*modify grant lentgh*/
    if(lenCnt%2)
    {
        if(value->grtMode){
            tmp = (lenCnt%(value->mpcpGrantLen/value->grtLenStep))*(value->grtLenStep) - value->mpcpGrantLen;
            printk("[%s %d] grant length: 0x%x\n", __FUNCTION__, __LINE__, tmp&0xffff);
            WRITE_REG_WORD(e_trx_adjust_time2, ((tmp<<16)+6));
        }else{
            WRITE_REG_WORD(e_trx_adjust_time2, ((value->grtLen)<<16)+6);
        }
    }else{
...
/*Reset mpcp cmd done bit(bfb66028[16]=1)*/
void eponRstMpcpCmdDone(void){
    REG_e_glb_cfg regRaw;
    memset(&regRaw, 0, sizeof(REG_e_glb_cfg));
	regRaw.Raw = READ_REG_WORD(e_glb_cfg);
    
	regRaw.Bits.epon_mac_sw_rst = 1;
	WRITE_REG_WORD(e_glb_cfg , regRaw.Raw );
   
	regRaw.Bits.epon_mac_sw_rst = 0;
	WRITE_REG_WORD(e_glb_cfg , regRaw.Raw);
...
void eponMpcpDiscvGateIntHandler(unsigned long data){
	...
	for(llidIndex = 0; llidIndex< EPON_1G_MAX_LLID_NUM; llidIndex++, llidEntry_p++){
		...
		if(llidEntry_p->enableFlag== XPON_DISABLE){
         ...
         if (gpEponPriv->mpcpInterruptMode){
            if(((llidEntry_p->eponMpcp.eponDiscFsm.mpcpDiscState == MPCP_STATE_REGISTER_REQUEST) ||
                (llidEntry_p->eponMpcp.eponDiscFsm.mpcpDiscState == MPCP_STATE_REGISTER_ACK)) 
                && eponMacCheckRegreqDoneBit() <0){
                eponRstMpcpCmdDone();
                dbgtoMem(EPON_DEBUG_LEVEL_NOTIFY, "%s : llid %d Reset MPCP Cmd done bit",__FUNCTION__,llidIndex);
                llidEntry_p->eponMpcp.eponDiscFsm.registered = FALSE;
                llidEntry_p->eponMpcp.eponDiscFsm.mpcpDiscState = MPCP_STATE_REGISTERING;
                //dbgtoMem(EPON_DEBUG_LEVEL_NOTIFY, "Ignore current Discovery Gate message");
                return;
            }
    	}
		...
		{
            xmcs_report_event(XMCS_EVENT_TYPE_EPON,XMCS_EVENT_EPON_REGISTERING,llidIndex);
			if(llidEntry_p->eponMpcp.eponDiscFsm.mpcpDiscState == MPCP_STATE_REGISTER_REQUEST){
			...
			if(llidEntry_p->eponMpcp.eponDiscFsm.mpcpReqRetryFlag > 0 ){
			    ...
			    /*sw modify disc gate grant length*/
			    if(gpEponPriv->eponRdmMfy.grtFlag){
            	    eponSetGrantLength();
            	}
				...
				if((eponMacCheckRegreqDoneBit() <0) ){
					dbgtoMem(EPON_DEBUG_LEVEL_ERR, "Epon MPCP DiscvGate IntHandler done bit not ready");
			...
			tmp = READ_REG_WORD(g_epon_llid_dscv_stat[llidIndex]) ;
			tmp &= 0x3fffffff;
			WRITE_REG_WORD(g_epon_llid_dscv_stat[llidIndex] , tmp);	
            ...
            dbgtoMem(EPON_DEBUG_LEVEL_NOTIFY, "%s : Ready to send Register Request message.",__FUNCTION__);
			...
			if(epon_random_delay_test)
			{
			    random_result[random_dly_cnt] = READ_REG_WORD(e_dscv_rdm_dly);
			    random_dly_cnt++;
			    if(random_dly_cnt >= random_delay_test_cnt || random_dly_cnt >= sizeof(random_result)/sizeof(int))
			    {
			        epon_random_delay_test = 0;
			        printk("random delay test count [%d] greater %d, or greater 2000\n", random_dly_cnt, random_delay_test_cnt);
			    }
...
int eponMpcpRgstAckIntHandler(void)
{
    ...
    }else{ //mpcpDiscFsm->mpcpDiscMacr_RgstrAck == NACK
        /*set Burst mode */
...
static int eponMpcpDscvFsmPendingHandler(__u8 llidIndex){
	...
	if (!gpEponPriv->mpcpInterruptMode)
	{
        ...
        while(1){
            ...
            if(cnt > 1000000){
                dbgtoMem(EPON_DEBUG_LEVEL_NOTIFY, "eponMpcpDscvFsmPendingHandler polling exceed 1000000 times !!!");
...
int eponMpcpRgstIntHandler(__u8 llidIndex){
	...
	llidDscvSts.Raw = READ_REG_WORD(g_epon_llid_dscv_stat[llidIndex]);
	...
	if(mpcpDiscFsm->mpcpDiscState == MPCP_STATE_REGISTER_REQUEST){
        ...
        /*restore grant length*/
        tmp = READ_REG_WORD(e_trx_adjust_time2);
        tmp &= 0xffff;
        WRITE_REG_WORD(e_trx_adjust_time2, tmp);
		...
		if((MPCP_REG_DE_REGISTER == llidDscvSts.Bits.rgstrFlgSts )|| (MPCP_REG_NACK== llidDscvSts.Bits.rgstrFlgSts ) ){
			...
			else
			{
				...
				dbgtoMem(EPON_DEBUG_LEVEL_ERR, "%s[%d]received deregister, stop timer.\n", __FUNCTION__, __LINE__);
			...
			change_pon_led_status(ALARM_LED_OFF);
#endif	
			if(MPCP_STATE_DENIED != mpcpDiscFsm->mpcpDiscState)
			{
				tasklet_schedule(&eponStartTask);
				dbgtoMem(EPON_DEBUG_LEVEL_NOTIFY, "ONU receive DEREGISTER ,do epon mac eponStart");
			}
			/* set burst mode */
			eponFirmwareSetMode(0);	
		}
...
void eponMacTxOamFav(__u32 value){
...
int eponMpcpLocalDergstr(__u8 llidIndex){
	...
	{
		dbgtoMem(EPON_DEBUG_LEVEL_ERR, "ERR: eponMpcpLocalDergstr llidindex = %d is invalid, not greater %d\n",
							llidIndex, EPON_1G_MAX_LLID_NUM - 1);
		return -1;
	}
	llidEntry_p = &(eponDrv.eponLlidEntry[llidIndex]);
	mpcpDiscFsm = &(llidEntry_p->eponMpcp.eponDiscFsm);
	...
	if(mpcpDiscFsm->mpcpDiscState ==  MPCP_STATE_REGISTERED){
		...
		if ((XPON_PHY_GET(PON_GET_PHY_LOS_STATUS) == PHY_NO_LOS_HAPPEN)){
			while(1){
				dscvCtrl.Raw = READ_REG_WORD(e_llid_dscvry_ctrl);
				if(dscvCtrl.Bits.mpcp_cmd_done == 1)
				{
...
int eponMpcpSetDiscvRgstAck(__u8 llidIndex , __u8 rgstAckFlag){
	...
	RgstrAckFlag[llidIndex] = rgstAckFlag;
	return 0;
}
...
int eponMpcpRegisterDone(void)
{
	...
	if(XPON_PHY_GET(PON_GET_PHY_MODE) != PHY_EPON_CONFIG) 
    {
        return 0;
    }
    ...
    llidEntry_p = &(eponDrv.eponLlidEntry[llidIndex]);
    mpcpDiscFsm = &(llidEntry_p->eponMpcp.eponDiscFsm);
    return mpcpDiscFsm->mpcpDiscState;
