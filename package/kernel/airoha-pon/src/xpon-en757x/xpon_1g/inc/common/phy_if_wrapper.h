...
void check_sync_result(TIMER_FUN_PAAM arg);

static inline void start_phy_mode_auto_detection(void)
{
    gpPhyData->detect_status = XPON_PHY_SYNC_STATUS_NOT_START ;
    XPON_START_TIMER(gpPhyData->check_sync_timer,gpPhyData->check_sync_timer_value);/* 1s */
}
...
static inline int XPON_PHY_SET(uint cmd_id)
{
	if(isEN7580 || isEN7581 || isAN7583)
	{
		if((TRUE == XPON_PHY_GET_API(PON_GET_PHY_INIT_STATUS,NULL)) || (cmd_id == PON_SET_PHY_START))
		{	
...
static inline int XPON_PHY_TX_VCC_ENABLE(void)
{
    int data = PHY_ENABLE;
    return XPON_PHY_SET_API(PON_SET_PHY_TRANS_VCC, &data);       
}

static inline int XPON_PHY_TX_VCC_DISABLE(void)
{
    int data = PHY_DISABLE;
    return XPON_PHY_SET_API(PON_SET_PHY_TRANS_VCC, &data);       
}
...
static inline void XPON_PHY_NOTIFY_EVENT(int event)
{
	if(xpon_check_emergence_state() == FALSE || event == PHY_XPON_STATE_NOTIFY_DYINGGASP)
	{
		PON_MSG(MSG_INT, "%s not in emergence_state or event dyinggasp \n",__FUNCTION__) ;
    	XPON_PHY_SET_API(PON_SET_PHY_NOTIFY_EVENT, &event);
	}
}
