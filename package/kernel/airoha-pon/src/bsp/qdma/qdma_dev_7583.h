...
ECONET SOFTWARE.
***************************************************************/
#ifndef _QDMA_DEV_7583_H_
#define _QDMA_DEV_7583_H_
...
/*end define for support change qdma rx ring dscp num dynamically*/
    ...
    #define CONFIG_HWFWD_DRAM_DSCP_NUM         	(49152) //DRAM:48k
#endif

    #define CONFIG_HWFWD_DSCP_NUM_MAX           (16384)
    #define CONFIG_HWFWD_SRAM_DSCP_NUM_MAX      (16384) //SRAM:16k
    #define CONFIG_HWFWD_DRAM_DSCP_NUM_MAX      (49152) //DRAM:48k
...
#ifdef __BIG_ENDIAN
#define GLB_CFG_VALUE GLB_CFG_DSCP_BYTE_SWAP | GLB_CFG_PAYLOAD_BYTE_SWAP | (DMA_MAX_ISSUE_3<<GLB_CFG_MAX_ISSUE_NUM_SHIFT) | (GLB_CFG_CPU_TXR_ROUND_ROBIN) | ((PREFER_CTX_FWD<<GLB_CFG_DMA_PREFERENCE_SHIFT)&GLB_CFG_DMA_PREFERENCE_MASK)|GLB_CFG_MAX_ISSUE_NUM_BIT2
#else
#define GLB_CFG_VALUE GLB_CFG_PAYLOAD_BYTE_SWAP | (DMA_MAX_ISSUE_3<<GLB_CFG_MAX_ISSUE_NUM_SHIFT) | (GLB_CFG_CPU_TXR_ROUND_ROBIN) | ((PREFER_CTX_FWD<<GLB_CFG_DMA_PREFERENCE_SHIFT)&GLB_CFG_DMA_PREFERENCE_MASK)|GLB_CFG_MAX_ISSUE_NUM_BIT2
#endif
...
#define qdmaSetInxColorResCfg(base,val)		IO_SREG(QDMA_CSR_INX_COLOR_RES_CFG(base), val)
#define qdmaSetInxColorResCfg1(base,val)	IO_SREG(QDMA_CSR_INX_COLOR_RES_CFG1(base), val)

#define qdmaGetInxColorResCfg(base)		IO_GREG(QDMA_CSR_INX_COLOR_RES_CFG(base))
#define qdmaGetInxColorResCfg1(base)	IO_GREG(QDMA_CSR_INX_COLOR_RES_CFG1(base))
...
static inline void qdma_set_txmsg_pad(txMsgWord_t* txMsg,uint pad)
{
    txMsg->pTxMsgW0->pad = pad&0x1;
    return;
}

static inline void qdma_set_txmsg_uco(txMsgWord_t* txMsg,uint uco)
{
    txMsg->pTxMsgW0->tuco = uco&0x1;
...
static inline void qdma_init_trtcm(uint base,uint* p_cfgBase,uint* p_bucketByteUnit,uint* p_bucketPacketUnit)
{
	...
	p_cfgBase[EGRESS_QUEUE_RATELIMIT] = QDMA_CSR_EGRESS_QUEUE_RATE_CFG(base);
	p_bucketByteUnit[EGRESS_QUEUE_RATELIMIT] = 1024;
	p_bucketPacketUnit[EGRESS_QUEUE_RATELIMIT] = 16;
