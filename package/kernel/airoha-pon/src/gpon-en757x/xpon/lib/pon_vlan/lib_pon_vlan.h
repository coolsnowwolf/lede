...
extern int pon_enable_vlan_rule(void);//0:success -1:error
extern int pon_disable_vlan_rule(void);//0:success -1:error

extern int pon_enable_veip_vlan_rule(void);//0:success -1:error
extern int pon_disable_veip_vlan_rule(void);//0:success -1:error

extern int pon_get_vlan_status(void);//0:disable 1:enable -1:error

extern int pon_add_vlan_rule(int port, pon_vlan_rule rule);//0:success -1:error
extern int pon_del_vlan_rule(int port, int index);//0:success -1:error index:0~31
extern int pon_clean_vlan_rule(int port);//0:success -1:error
extern int pon_clean_hwnat(void);//0:success -1:error
extern int pon_clean_all_vlan_rule(void);//0:success -1:error
...
extern int pon_get_vlan_rule(int port,pon_vlan_rule * rule);//0~32:rule num -1:error
extern int pon_get_vlan_rule_index(int port,pon_vlan_rule * rule);//0~31:rule index,32:no rule -1:error
	
extern int pon_set_vlan_downstream_mode(int port,int mode);//0:success -1:error
extern int pon_get_vlan_downstream_mode(int port);//0~1:mode -1:error

extern int pon_add_special_tpid(__u16 TPID);
extern int pon_del_special_tpid(__u16 TPID);

extern int pon_set_vlan_tpid(int port,__u16 inTPID, __u16 outTPID);//0:success -1:error
extern int pon_get_vlan_input_tpid(int port);//0x8100,0x88a8,0x9100:tpid value -1:error
extern int pon_get_vlan_output_tpid(int port);//0x8100,0x88a8,0x9100:tpid value -1:error

extern int pon_set_vlan_dscp_map(int port, __u32 * map);//0:success -1:error
extern int pon_get_vlan_dscp_map(int port, __u32 * map);//0:success -1:error

extern int pon_enable_default_vlan_rule(int port);//0:success -1:error (port can't be 40)
extern int pon_disable_default_vlan_rule(int port);//0:success -1:error (port can't be 40)
extern int pon_get_default_vlan_status(int port);//0,1:status  -1:error

extern int pon_enable_igmp_vlan_rule(void);//0:success -1:error
extern int pon_disable_igmp_vlan_rule(void);//0:success -1:error
extern int pon_get_igmp_vlan_status(void);//0,1:status -1:error

extern int pon_set_igmp_vlan_mode(int port, int mode);//0:success -1:error (port can't be 40)
extern int pon_get_igmp_vlan_mode(int port);//0~3:mode  -1:error

extern int pon_set_igmp_vlan_tci(int port, __u16 tci);//0:success -1:error (port can't be 40)
extern int pon_get_igmp_vlan_tci(int port);//0~0xFF:tci value -1:error

extern int pon_vlan_set_default(int port);//0:success -1:error
extern int pon_igmp_set_default(int port);//0:success -1:error (port can't be 40)

extern int pon_enable_interface_bind_vlan();//0:success -1:error
extern int pon_disable_interface_bind_vlan();//0:success -1:error
extern int pon_set_interface_vlan(char * ifname,int vid);//0:success -1:error
extern int pon_get_interface_vlan(char * ifname);//-1:error 0-4095:vid 4096:transparent 4097:block

extern int pon_set_pcp_mode(int mode);//0:success -1:error
extern int pon_get_pcp_mode(void);//-1:error  0~3:mode

extern int pon_enable_mac_vlan(int port);//0:success -1:error
extern int pon_disable_mac_vlan(int port);//0:success -1:error
extern int pon_get_mac_vlan_status(int port);//0,1:status -1:error
extern int pon_set_mac_vlan_time(int time);//0:success -1:error
extern int pon_get_mac_vlan_time();//0~3600:time -1:error

extern int pon_enable_user_group(void);//-1:error
extern int pon_disable_user_group(void);//-1:error
extern int pon_user_group_get_status(void);//-1:error 0:disable 1:enable
extern int pon_set_user_group(__u8 port,__u32 group);//port:0~3:lan port1~4  group:0~0xffff instanceID,0xffffffff not in a group
extern int pon_get_user_group(__u8 port,__u32 * group);
extern int pon_clean_all_user_group(void);//-1:error
...
/* -----------------------------OUT Interfaces----------------------------- */
extern int addGponVlanFilterRule(gponVlanFilterIoctl_ptr vlanFilterRule_ptr);
extern int getGponVlanFilterRule(gponVlanFilterIoctl_ptr vlanFilterRule_ptr);
extern int delGponVlanFilterRule(__u16 port, __u8 portType);
extern int cleanAllGponVlanFilterRule(void);
extern int displayAllGponVlanFilterRule(void);

// support EPON OAM 

#define MAX_VLAN_RUELS_NUM 33
extern int eponClearVlanRules(__u8 port);
extern int eponSetVlanTransparent(__u8 port);
extern int eponBlockAllVlanTag(__u8 port);
extern int eponSetDefVlanTag(__u8 port, uint tag);
extern int eponGetDefVlanTag(__u8 port, uint *tag);
extern int eponAddTranslateVlanTag(__u8 port, uint old_tag, uint new_tag);
extern int eponGetTranslateVlanTag(__u8 port, __u8 num, uint *old_tags, uint *new_tags);
extern int eponAddN1TranslateVlanTag(__u8 port, __u8 num, uint *old_tags, uint new_tag);
extern int eponGetN1TranslateVlanTag(__u8 port, uint *old_nums, uint *old_tags, uint *new_num, uint *new_tags);
extern int eponAddTrunkVlanTag(__u8 port, uint tag);
extern int eponGetTrunkVlanTag(__u8 port, __u8 num, uint *tags);
