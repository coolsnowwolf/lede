...
#define PHYYNC_ACTIVE_LOW BIT(1)
...
#define DSI_VID_PHYA_LINES 0x54
...
#define FTD330_TXREQUESTCLKHS BIT(0)

#define DSI_FTD330_TMR_LPCLK_CFG 0x98
#define FTD330_CLKHS2LP_TIME(lbcc) (((lbcc)&0x3ff) << 16)
#define FTD330_CLKLP2HS_TIME(lbcc) ((lbcc)&0x3ff)

#define DSI_FTD330_TMR_CFG 0x9c
#define FTD330_HS2LP_TIME(lbcc) (((lbcc)&0x3ff) << 16)
#define FTD330_LP2HS_TIME(lbcc) ((lbcc)&0x3ff)

#define DSI_FTD330_RSTZ 0xa0
#define FTD330_DISFORCEPLL 0
#define FTD330_ENFORCEPLL BIT(3)
#define FTD330_DISABLECLK 0
#define FTD330_ENABLECLK BIT(2)
#define FTD330_RSTZ 0
#define FTD330_UNRSTZ BIT(1)
#define FTD330_SHUTDOWNZ 0
#define FTD330_UNSHUTDOWNZ BIT(0)

#define DSI_FTD330_IF_CFG 0xa4
#define FTD330_STOP_WAIT_TIME(cycle) (((cycle)&0xff) << 8)
#define N_LANES(n) (((n)-1) & 0x3)

#define DSI_FTD330_ULPS_CTRL 0xa8
#define DSI_FTD330_TX_TRIGGERS 0xac

#define DSI_FTD330_STATUS 0xb0
#define FTD330_STOP_STATE_CLK_LANE BIT(2)
#define FTD330_LOCK BIT(0)
...
#define DSI_FTD330_TMR_RD_CFG 0xf4
#define MAX_RD_TIME(lbcc) ((lbcc)&0x7fff)

#define FTD330_STATUS_TIMEOUT_US 10000
...
static inline void dsi_write(struct dw_mipi_dsi *dsi, u32 reg, u32 val)
{
	...
	pr_info("dsi-w-addr:0x%x 0x%x\n", reg, value);
}

static inline u32 dsi_read(struct dw_mipi_dsi *dsi, u32 reg)
{
	u32 value = readl(dsi->base + reg);

	pr_info("dsi-r-addr:0x%x 0x%x\n", reg, value);
	return value;
}
...
static void dw_mipi_message_config(struct dw_mipi_dsi *dsi, const struct mipi_dsi_msg *msg)
{
	...
	dsi_write(dsi, DSI_LPCLK_CTRL, lpm ? 0 : FTD330_TXREQUESTCLKHS);
...
{
	dsi_write(dsi, DSI_VID_MODE_CFG, ENABLE_LOW_POWER | VID_MODE_TYPE_BURST);
}
...
static void dw_mipi_dsi_set_mode(struct dw_mipi_dsi *dsi, unsigned long mode_flags)
{
	...
	if (mode_flags & MIPI_DSI_MODE_VIDEO) {
		...
		dsi_write(dsi, DSI_LPCLK_CTRL, FTD330_TXREQUESTCLKHS);
	} else {
...
static void dw_mipi_dsi_init(struct dw_mipi_dsi *dsi)
{
	 ...
	 */
	dsi_write(dsi, DSI_CLKMGR_CFG, TO_CLK_DIVISION(10) | TX_ESC_CLK_DIVISION(esc_clk_division));
}
...
static void dw_mipi_dsi_dpi_config(struct dw_mipi_dsi *dsi, const struct drm_display_mode *mode)
{
	...
	if (!(mode->flags & DRM_MODE_FLAG_PPHYYNC))
		val |= PHYYNC_ACTIVE_LOW;
	...
	/*
	 * TODO dw drv improvements
	 * largest packet sizes during hfp or during phya/vpb/vfp
	 * should be computed according to byte lane, lane number and only
	 * if sending lp cmds in high speed is enable (FTD330_TXREQUESTCLKHS)
	 */
...
static void dw_mipi_dsi_timing_config(struct dw_mipi_dsi *dsi, const struct drm_display_mode *mode)
{
	...
	dsi_write(dsi, DSI_VID_PHYA_LINES, mode->phyync_end - mode->phyync_start);
	dsi_write(dsi, DSI_VID_VFP_LINES, mode->phyync_start - mode->vdisplay);
	dsi_write(dsi, DSI_VID_VBP_LINES, mode->vtotal - mode->phyync_end);
}
...
static void dw_mipi_dsi_dphy_timing_config(struct dw_mipi_dsi *dsi)
{
	 ...
	 * note: DSI_FTD330_TMR_CFG.MAX_RD_TIME should be in line with
	 * DSI_CMD_MODE_CFG.MAX_RD_PKT_SIZE_LP (see CMD_MODE_ALL_LP)
	 */

	dsi_write(dsi, DSI_FTD330_TMR_CFG, FTD330_HS2LP_TIME(0x40) | FTD330_LP2HS_TIME(0x40));
	dsi_write(dsi, DSI_FTD330_TMR_RD_CFG, MAX_RD_TIME(10000));

	dsi_write(dsi, DSI_FTD330_TMR_LPCLK_CFG, FTD330_CLKHS2LP_TIME(0x40) | FTD330_CLKLP2HS_TIME(0x40));
}
...
static void dw_mipi_dsi_dphy_interface_config(struct dw_mipi_dsi *dsi)
{
	 ...
	 */
	dsi_write(dsi, DSI_FTD330_IF_CFG, FTD330_STOP_WAIT_TIME(0x20) | N_LANES(dsi->lanes));
}

static void dw_mipi_dsi_dphy_init(struct dw_mipi_dsi *dsi)
{
	/* Clear PHY state */
	dsi_write(dsi, DSI_FTD330_RSTZ, FTD330_DISFORCEPLL | FTD330_DISABLECLK | FTD330_RSTZ | FTD330_SHUTDOWNZ);
}
...
static void dw_mipi_dsi_dphy_enable(struct dw_mipi_dsi *dsi)
{
	...
	ftd330_power_on(dsi->dphy);
	dsi_write(dsi, DSI_FTD330_RSTZ, FTD330_ENFORCEPLL | FTD330_ENABLECLK | FTD330_UNSHUTDOWNZ);
	usleep_range(500, 600);
	dsi_write(dsi, DSI_FTD330_RSTZ, FTD330_ENFORCEPLL | FTD330_ENABLECLK | FTD330_UNRSTZ | FTD330_UNSHUTDOWNZ);

	ret = readl_poll_timeout(dsi->base + DSI_FTD330_STATUS, val, val & FTD330_LOCK, 1000,
				 FTD330_STATUS_TIMEOUT_US);
	if (ret)
		DRM_DEBUG_DRIVER("failed to wait phy lock state\n");

	ret = readl_poll_timeout(dsi->base + DSI_FTD330_STATUS, val, val & FTD330_STOP_STATE_CLK_LANE,
				 1000, FTD330_STATUS_TIMEOUT_US);
	if (ret)
...
{
	struct ftd330_configure_opts_mipi_dphy dphy_cfg;
	int bpp = mipi_dsi_pixel_format_to_bpp(dsi->format);

	ftd330_init(dsi->dphy);
	ftd330_mipi_dphy_get_default_config(mode->clock * 1000, bpp, dsi->lanes, &dphy_cfg);
	ftd330_validate(dsi->dphy, FTD330_MODE_MIPI_DPHY, 0, (union ftd330_configure_opts *)&dphy_cfg);
	ftd330_configure(dsi->dphy, (union ftd330_configure_opts *)&dphy_cfg);
			    ...
			    const struct drm_display_mode *adjusted_mode)
{
	...
	struct drm_display_mode *new_mode = drm_mode_duplicate(bridge->dev, adjusted_mode);
...
static void bridge_enable(struct drm_bridge *bridge)
{
	...
	if (primary->secondary_dsi)
		dw_mipi_dsi_set_mode(primary->secondary_dsi, MIPI_DSI_MODE_VIDEO);
}

static void dw_mipi_dsi_disable(struct dw_mipi_dsi *dsi)
{
	ftd330_power_off(dsi->dphy);

	dsi_write(dsi, DSI_PWR_UP, RESET);
	dsi_write(dsi, DSI_FTD330_RSTZ, FTD330_RSTZ);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 7, 0)
	return drm_bridge_attach(bridge->encoder, primary->panel_bridge, bridge, 0);
#else
	return drm_bridge_attach(bridge->encoder, primary->panel_bridge, bridge);
#endif
}
...
static int dsi_attach_primary(struct dw_mipi_dsi *secondary, struct device *dev)
{
	...
	if (!of_device_is_compatible(dev->of_node, "phytium,dw-mipi-dsi"))
		return -EINVAL;
...
static int secondary_bind(struct dw_mipi_dsi *dsi)
{
	...
	np = of_find_compatible_node(NULL, NULL, "phytium,dw-mipi-dsi");
	if (!np)
		return -ENODEV;
...
static const struct of_device_id dw_mipi_dsi_dt_match[] = {
	{ .compatible = "phytium,dw-mipi-dsi", .data = &primary },
	{ .compatible = "phytium,dw-mipi-dsi-2nd", .data = &secondary },
	{},
};
...
static int dsi_probe(struct platform_device *pdev)
{
	...
	dsi->dphy = devm_ftd330_get(dev, "dphy");
	if (IS_ERR(dsi->dphy))
