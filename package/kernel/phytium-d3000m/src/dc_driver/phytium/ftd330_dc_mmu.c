...
static int _allocate_memory(u32 bytes, void **memory)
{
	void *mem = NULL;
...
static int _allocate_stlb(dc_mmu_stlb_pt *stlb)
{
...
static int _allocate_all_stlb(struct device *dev, dc_mmu_stlb_pt *stlb)
{
	dc_mmu_stlb_pt stlb_t = NULL;
	void *mem = NULL;
	void *cookie = NULL;
...
static int _setup_process_address_space(struct device *dev, dc_mmu_pt mmu)
{
	u32 *map = NULL;
	u32 free, i;
	...
	ret = _allocate_memory(mmu->page_table_size, (void **)&mmu->map_logical);
	if (ret) {
...
/* Initialize free area,mark first pageas used to skip it.so that
 * avoid virtual address = 0
 */
	...
	if (ret)
		return ret;
	mmu->all_stlb_logical = all_stlb->logical;
	mmu->all_stlb_physical = all_stlb->physBase;
	mmu->all_stlb_size = all_stlb->size;
...
void dc_mmu_deconstruct(struct device *dev, dc_mmu_pt mmu)
{
	if (!mmu_construct)
		return;
	if (mmu->mtlb_logical)
		dma_free_wc(dev, mmu->mtlb_size, mmu->mtlb_logical, mmu->mtlb_physical);
	if (mmu->safe_page_logical)
		dma_free_wc(dev, mmu->safe_page_size, mmu->safe_page_logical,
			    mmu->safe_page_physical);
	if (mmu->all_stlb_logical)
		dma_free_wc(dev, mmu->all_stlb_size, mmu->all_stlb_logical, mmu->all_stlb_physical);
	mmu_construct = false;
}

static int dc_mmu_get_page_entry(dc_mmu_pt mmu, u32 address, u32 **page_table)
{
	dc_mmu_stlb_pt stlb;
	dc_mmu_stlb_pt *stlbs = (dc_mmu_stlb_pt *)mmu->stlbs;
...
static int _link(dc_mmu_pt mmu, u32 index, u32 node)
{
...
static int _add_free(dc_mmu_pt mmu, u32 index, u32 node, u32 count)
{
	u32 *map = mmu->map_logical;
...
/* Collect free nodes */
static int _collect(dc_mmu_pt mmu)
{
	u32 *map = mmu->map_logical;
...
static int _fill_page_table(u32 *page_table, u32 page_count, u32 entry_value)
{
	u32 i;
...
static int dc_mmu_allocate_pages(dc_mmu_pt mmu, u32 page_count, u32 *address)
{
	...
	case DC_MMU_FREE:
		...
		case 1:
			...
			_write_page_entry(&map[index],
					  (_read_page_entry(&map[index + 1]) << 8) | DC_MMU_SINGLE);
			index++;
			break;
		default:
			/* FREE type node left */
			_write_page_entry(&map[index], (left << 8) | DC_MMU_FREE);
			index += left;
			break;
		}
		break;
...
static int dc_mmu_free_pages(dc_mmu_pt mmu, u32 address, u32 page_count)
{
	u32 *node;
...
static int dc_mmu_set_page(dc_mmu_pt mmu, u64 page_address, u32 *page_entry)
{
	u32 address_ext;
	u32 address;
...
static int dc_mmu_unmap_memory(dc_mmu_pt mmu, u32 gpu_address, u32 page_count)
{
	FTD330_LOG("dc unmap memory addr:0x%x\n", gpu_address);
				  ...
				  u32 page_count)
{
	...
	ret = dc_mmu_unmap_memory(mmu, gpu_address, page_count);
	if (ret)
		return ret;
