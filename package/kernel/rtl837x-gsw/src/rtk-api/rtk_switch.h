...
/* UTIL MACRO */
#define RTK_ERR_CHK(op)                                     \
do {                                                        \
    int errCode;                                          \
    if ((errCode = (op)) != RT_ERR_OK)                    \
        return errCode;                                     \
} while(0)
...
//rtk_api_ret_t rtk_fw_reset_flow_tgr(rtk_uint32 sdsid);
//rtk_api_ret_t rtk_fw_reset_flow_tgx(rtk_uint32 sds);
