...
do
    ...
    if [ $curl_code -eq 0 ];then
        ...
        if [ -n "$ssr_group" ]; then
            ...
            subscribe_log="['$LOGTIME'] $ssr_group 服务器订阅更新成功 服务器数量: ${#ssr_url[@]} 新增服务器: $subscribe_n 删除服务器: $subscribe_o"
            logger -st $log_name[$$] -p6 "$subscribe_log"
            uci commit $name
        else
            logger -st $log_name[$$] -p3 "['$LOGTIME'] ${subscribe_url[$o]} 订阅数据解析失败 无法获取 Group"
        fi
    else
        logger -st $log_name[$$] -p3 "['$LOGTIME'] ${subscribe_url[$o]} 订阅数据获取失败 错误代码: $curl_code"
    fi
done
