...
INT BndStrg_Init(PRTMP_ADAPTER pAd)
{
	...
	EnableRadioChstats(pAd); //configure register for channel busy time
	return ret_val;
}
...
VOID BndStrg_InsertNeighborRepIE(PRTMP_ADAPTER pAd, PBND_STRG_CLI_TABLE init_table)
{
	...
	CondensedPhyType = (pAd->CommonCfg.Channel > 14)?9:7; /* 7:2G,9:5G */
	/* our own info */
	BssidInfo.word = 0;
	...
	NeighborRepInfo.RegulatoryClass = get_regulatory_class(pAd, 0);
	NeighborRepInfo.ChNum = pAd->CommonCfg.Channel;
	...
	PEER_PROBE_REQ_PARAM * ProbeReqParam)
{
	...
	cli_event->Channel = table->Channel;
	...
	BOOLEAN bInfReady)
{
	...
	if (bInfReady) { /* Exec. by each interface up */
		...
		table->Channel = pAd->CommonCfg.Channel;
	...
	table->nss = pAd->CommonCfg.TxStream;
...
VOID BndStrg_ChannelLoadStatusRsp(PRTMP_ADAPTER pAd, PBND_STRG_CLI_TABLE table, BNDSTRG_MSG *msg)
{
	if (table->bInitialized == TRUE) {
		...
		chanload_status_rsp->chanload = (pAd->ApCfg.BndStrgOneSecChBusyTime * 100)/ONE_SEC_2_US;
		BndStrgSendMsg(pAd, &new_msg);
	}
}
...
{
	if (pAd->ApCfg.BndStrgTable[0].bEnabled ) {
	...
	PBND_STRG_CLI_TABLE table)
{
	...
	RRM_InsertNeighborRepIE(pAd, (Event->u.BTM_REQ_DATA.BTMReq + BTMReqLen), &FrameLen,
		sizeof(RRM_NEIGHBOR_REP_INFO)+3, NeighborReport->Bssid, (RRM_BSSID_INFO)NeighborReport->BssidInfo, 
		NeighborReport->RegulatoryClass, NeighborReport->ChNum, NeighborReport->PhyType);
	BTMReqLen += FrameLen;
	{
		ULONG TempLen = 0;
		UINT8 IEId = 3;
	...
	hex_dump("BndStrg_send_BTM_req Event length", (unsigned char *)Event->u.BTM_REQ_DATA.BTMReq,
...
VOID BndStrg_KickOutAllSta(PRTMP_ADAPTER pAd, UCHAR apidx, USHORT Reason)
{
