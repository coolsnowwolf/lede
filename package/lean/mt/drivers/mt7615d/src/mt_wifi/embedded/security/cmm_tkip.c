...
struct GNU_PACKED _IV_CONTROL_ {
	union GNU_PACKED {
		struct GNU_PACKED {
			UCHAR		rc0;
			UCHAR		rc1;
...
/*
	...
	Routine	Description:
		Convert from UCHAR[] to UINT32 in a portable way
...
/*
	...
	Routine	Description:
		Convert from UINT32 to UCHAR[] in a portable way

	Arguments:
      pDst			pointer to destination for convert UINT32 to UCHAR[]
