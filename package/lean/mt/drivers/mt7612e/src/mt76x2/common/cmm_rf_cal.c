	...
	IN UCHAR AntIdx)
{
	...
	for (check_loop = 0; check_loop < 2; check_loop++)
	{
		if (AntIdx == 0)
		{
			DBGPRINT(RT_DEBUG_INFO, ("DPD Scaling for TX0 !!!\n"));
			if (check_loop == 0)
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R186, 0x02);
			else
		...
		{
			DBGPRINT(RT_DEBUG_INFO, ("DPD Scaling for TX1 !!!\n"));
			if (check_loop == 0)
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R186, 0x04);
			else
	...
	for (check_loop = 0; check_loop < 2; check_loop++)
	{
		...
		for (index = 0; index < Max_Retry; index++)
		{
			while ((VGA_code_idx >= 0) && (VGA_code_idx <= 18))
			{
				...
				RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R188, &byteValue);
				AM_63 = byteValue;
				DBGPRINT(RT_DEBUG_INFO, ("AM_63 (%d)\n", AM_63));
				...
				{
					DBGPRINT(RT_DEBUG_INFO, ("Appropriate VGA Gain(%d), R66(%x)\n", RT6352_VGA_TABLE[VGA_code_idx].Register, VGA_code));
					DBGPRINT(RT_DEBUG_INFO, ("AM_63 (%d)\n", AM_63));
					break;
				}
			}
			...
			DBGPRINT(RT_DEBUG_INFO, ("AM_SUM = %d, AM_10 = %d !!!\n", AM_SUM, AM_10));
			DBGPRINT(RT_DEBUG_INFO, ("(AM_SUM - 5.5*AM_10) = (%d) @@\n", (AM_SUM - ((11 * AM_10) / 2))));
	...
	IN BOOLEAN bTxCal)
{
	...
	do {
		RtmpusecDelay(500);
		RT635xReadRFRegister(pAd, RF_BANK5, RF_R01, &rf_val);
		DBGPRINT(RT_DEBUG_INFO, ("ABB.R1 = %d, Cnt = %d \n", rf_val, cnt));
