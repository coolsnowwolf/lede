...
ifeq ($(CONFIG_PACKAGE_$(PKG_NAME)_INCLUDE_CUSTOM),y)
	$(INSTALL_CONF) ./root/etc/config/serverchan_custom $(1)/etc/config
	$(INSTALL_BIN) ./root/etc/init.d/sccustom $(1)/etc/init.d
	$(INSTALL_BIN) ./root/usr/bin/serverchan/sccustom $(1)/usr/bin/serverchan
	$(INSTALL_DATA)  ./luasrc/model/cbi/serverchan/custom.lua $(1)/usr/lib/lua/luci/model/cbi/serverchan
